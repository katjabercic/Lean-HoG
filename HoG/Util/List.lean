...
theorem all_distinct_get_injective_function {α : Type} [DecidableEq α] {l : List α}
...
theorem not_all_distinct_exists_duplicate {α : Type} [DecidableEq α] {l : List α} {len : 1 < l.length}:
  ¬l.all_distinct → ∃ (i j : Fin l.length), i ≠ j ∧ l.get i = l.get j := by
  intro h
  by_contra h'
  apply Iff.mp not_exists at h'
  have : ∀ (i j : Fin l.length), ¬ (i ≠ j ∧ get l i = get l j) := by
    intro i
    have ne := h' i
    apply Iff.mp not_exists at ne
    exact ne
  have : ∀ (i j : Fin l.length), i = j ∨ (get l i ≠ get l j) := by
    intros i j
    have foo := this i j
    apply Iff.mp not_and_or at foo
    simp [not_ne_iff, foo] at foo
    exact foo
  have exists_elem : ∃ (x : α), x ∈ l := by
    apply Iff.mp List.length_pos_iff_exists_mem
    linarith
  let ⟨x, x_in_l⟩ := exists_elem
  -- have l_not_nodup : ¬l.Nodup := by apply not_congr (Iff.mp all_distinct_iff_nodup h)
  sorry
