...
lemma witnessConnectedCondition (w : numComponentsWitness) : ∀ u v, w.c u = w.c v ↔ connected w.G u v := by
  intros u v
  apply Iff.intro
  · intro H
...
theorem witnessComponents : numComponentsWitness → numberOfConnectedComponents :=
  fun w =>
    {
      G := w.G,
