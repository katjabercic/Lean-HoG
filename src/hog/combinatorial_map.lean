...
def add_list_to_set_uniq : tset (lex ℕ ℕ) → list ℕ → option (tset (lex ℕ ℕ))
| acc [] := some acc
| acc (x :: []) := some acc
| acc (x :: y :: xs) := (
    match acc.elem (x, y) with 
    | tt := none
    | ff := add_list_to_set_uniq (acc.add (x, y)) (y :: xs)
    end
  )
def add_face_to_set_uniq : tset (lex ℕ ℕ) → list ℕ → option (tset (lex ℕ ℕ))
| acc [] := some acc 
| acc l@(x :: xs) := add_list_to_set_uniq acc (l ++ [x])
--| acc l@(x :: y :: xs) := add_list_to_set_uniq acc (l ++ [x])
def faces_unique_edges_aux : tset (lex ℕ ℕ) → list (list ℕ) → option (tset (lex ℕ ℕ)) 
| acc [] := some acc 
| acc (f :: fs) := (
    match add_face_to_set_uniq acc f with 
    | none := none
...
/- 
  Note: The computable version of composition_faces_consistent would use list.all, but 
  the version of Mathlib used by the project lacks lemmas, that are needed for proofs
-/
def connected_component_num (G : simple_irreflexive_graph) : ℕ := sorry
def isolated_vertex_num (G : simple_irreflexive_graph) : ℕ := sorry 

def genus {G : simple_irreflexive_graph} (M : combinatorial_map G) : ℚ :=
  1/2 * (G.edge_size - G.vertex_size - M.faces.length + 
    2 * connected_component_num G - isolated_vertex_num G)
    
def cycle_contains_key_opt (k : ℕ) : option cycle → bool 
| none := ff
| (some c) := c.next.contains_key k
def in_neighbors (G : simple_irreflexive_graph) (x y : ℕ) : Prop :=
...
lemma σ_neighbour_keys {G : simple_irreflexive_graph} {K : combinatorial_map G} :
...
theorem σ_edges_consistent {G : simple_irreflexive_graph} (K : combinatorial_map G) :
  ∀ x y, cycle_contains_key_opt y (K.σ.val_at x) ↔ in_neighbors G x y :=
begin
  intros x y,
  split,
  -- →
  ...
  { simp [σx, cycle_contains_key_opt] at y_in_σx,
    ...
    have σ_keys_neigh_keys := σ_neighbour_keys x x_in_σ,
    unfold in_neighbors,
    ...
    simp [←x_neigh_cond, in_neighbors._match_1],
  ...
  -- ←
  intro y_in_neigh_x,
  unfold in_neighbors at y_in_neigh_x,
  cases x_neigh_cond : (G.neighborhoods).val_at x with x_neigh,
  { simp [x_neigh_cond, in_neighbors._match_1] at y_in_neigh_x, apply false.elim, assumption },
  { simp [x_neigh_cond, in_neighbors._match_1] at y_in_neigh_x,
    cases σx : K.σ.val_at x with c,
...
theorem σα_maps_neighbours {G : simple_irreflexive_graph} (K : combinatorial_map G) :
  ∀ x y z, σα_maps K.σ x y z = tt → in_neighbors G y x :=
begin 
...
theorem edge_in_σα_list {G : simple_irreflexive_graph} (K : combinatorial_map G) :
  ∀ xs x', is_list_of_σα K.σ (xs ++ [x']) → ∀ x y, edge_in_list (x, y) xs → in_neighbors G y x :=
begin
  intros xs x',
...
theorem edge_in_σα_face {G : simple_irreflexive_graph} (K : combinatorial_map G) :
  ∀ xs, is_face_of_σα K.σ xs → ∀ x y, edge_in_face (x, y) xs → in_neighbors G y x :=
begin
  intro xs,
  cases xs with x' xs,
  ...
  { cases xs with y' xs,
    { simp [is_face_of_σα, edge_in_face] },
    { intros h x y h',
      apply edge_in_σα_list K (x' :: y' :: xs ++ [x']) y', 
      { simp [is_face_of_σα] at h, simp, assumption },
      { apply edge_in_list_remove_repeat, simp [edge_in_face] at h', assumption }
    }
  }
end
theorem face_pair_is_edge {G : simple_irreflexive_graph} (K : combinatorial_map G) :
  ∀ x y, (∃ f, f ∈ K.faces ∧ edge_in_face (x, y) f) → in_neighbors G y x :=
begin
  intros x y,
...
lemma set_in_uniq_list_set {t : tset (lex ℕ ℕ)} (acc : tset (lex ℕ ℕ)) (l : list ℕ) :
  add_list_to_set_uniq acc l = some t → ∀ e, acc.elem e → t.elem e :=
begin
  intros add_l_to_set e e_in_acc,
  induction l with x xs generalizing acc,
  { simp [add_list_to_set_uniq] at add_l_to_set,
    rw ←add_l_to_set,
    assumption 
  },
  { cases xs with y xs,
    ...
    { simp [add_list_to_set_uniq] at add_l_to_set,
      cases acc.elem (x, y),
      { simp [add_list_to_set_uniq._match_1] at add_l_to_set,
...
theorem list_in_uniq_set {t : tset (lex ℕ ℕ)} (acc : tset (lex ℕ ℕ)) (l : list ℕ) :
  add_list_to_set_uniq acc l = some t → ∀ e, edge_in_list e l → t.elem e :=
begin
  ...
  { cases xs with y xs,
    ...
    { simp [edge_in_list] at e_in_l,
      ...
      { simp [add_list_to_set_uniq._match_1] at add_l_to_set,
        ...
        { apply set_in_uniq_list_set (acc.add (x, y)) (y :: xs),
          assumption,
          apply tset.elem_add, assumption,
        },
...
lemma list_not_in_acc_if_uniq {t : tset (lex ℕ ℕ)} (acc : tset (lex ℕ ℕ)) (l : list ℕ) :
  add_list_to_set_uniq acc l = some t → ∀ e, edge_in_list e l → ¬ (acc.elem e) :=
begin 
  ...
  { cases xs with y xs,
    { simp [edge_in_list] at e_in_l, assumption },
    { simp [add_list_to_set_uniq] at add_l,
      simp [edge_in_list] at e_in_l,
      cases e_in_l with e_eq_xy,
      ...
      { cases acc.elem (x, y),
        ...
        { simp [add_list_to_set_uniq] at add_l, assumption }
      }
    }
  }
end

lemma face_not_in_acc_if_uniq {t : tset (lex ℕ ℕ)} (acc : tset (lex ℕ ℕ)) (l : list ℕ) :
  add_face_to_set_uniq acc l = some t → ∀ e, edge_in_face e l → ¬ (acc.elem e) :=
begin
  cases l with x xs,
  { simp [add_face_to_set_uniq, edge_in_face] },
  { cases xs with y xs,
    { simp [edge_in_face] },
    { unfold edge_in_face, unfold add_face_to_set_uniq,
      intros add_l e e_in,
      apply list_not_in_acc_if_uniq acc (x :: y :: xs ++ [x]) add_l,
      simp, simp at e_in,
      apply edge_in_list_remove_repeat e_in,
    }
  }
end
lemma set_in_uniq_face_set {t : tset (lex ℕ ℕ)} (acc : tset (lex ℕ ℕ)) (f : list ℕ) : 
  add_face_to_set_uniq acc f = some t → ∀ e, acc.elem e → t.elem e :=
begin 
  cases f with x xs,
  { simp [add_face_to_set_uniq],
    intro acc_eq_t,
    simp [acc_eq_t]
  },
  { simp [add_face_to_set_uniq],
    apply set_in_uniq_list_set
  }
end
lemma face_in_uniq_set {t : tset (lex ℕ ℕ)} (acc : tset (lex ℕ ℕ)) (f : list ℕ) :
  add_face_to_set_uniq acc f = some t → ∀ e, edge_in_face e f → t.elem e :=
begin
  cases f with x xs,
  { simp [add_face_to_set_uniq, edge_in_face] },
  { simp [add_face_to_set_uniq, edge_in_face],
    ...
    { simp [add_face_to_set_uniq, edge_in_face],
      ...
      apply list_in_uniq_set acc _ add_cond e e_in'
    }
  }
end
theorem edge_in_face_not_later {t : tset (lex ℕ ℕ)} (acc : tset (lex ℕ ℕ)) 
...
begin
  ...
  cases h : add_face_to_set_uniq acc f with t',
  ...
  { rw h at unique_cond,
    ...
    { simp at g_in_fs,
      cases h' : add_face_to_set_uniq t' f' with t'',
      ...
      { simp [faces_unique_edges_aux] at unique_cond,
        ...
        { cases e_in with e_in_f e_in_acc,
          { exact face_in_uniq_set acc f h e e_in_f },
          { exact set_in_uniq_face_set acc f h e e_in_acc }
        },
        cases g_in_fs with g_eq_f',
        { rw g_eq_f' at e_in_g,
          apply face_not_in_acc_if_uniq t' f' h' e e_in_g e_in_t'
        },
...
theorem edge_in_uniq_face_aux  {t : tset (lex ℕ ℕ)} (acc : tset (lex ℕ ℕ)) 
...
theorem edge_in_uniq_face (faces : list (list ℕ)) : faces_unique_edges faces = tt →
  ∀ i j : fin (faces.length), i.val ≤ j.val →
    ∀ e, edge_in_face e (list.nth_le faces i.val i.property) →
          edge_in_face e (list.nth_le faces j.val j.property) →
    i.val = j.val :=
