...
class ConnectedComponentsCertificate():
    ...
    @staticmethod
    def _dict_to_sorted_pairs(d : Dict[int, int]):
        return sorted([[v, c] for v, c in d.items()])

    def to_json(self):
        return {
            "val" : self.val,
            "component" : self._dict_to_sorted_pairs(self.component),
            "next" : self._dict_to_sorted_pairs(self.nextVertex),
            "distToRoot" : self._dict_to_sorted_pairs(self.distToRoot),
            "root" : self._dict_to_sorted_pairs(self.root),
        }
