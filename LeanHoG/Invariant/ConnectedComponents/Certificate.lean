...
def connectedComponentsCertificateOfData (G : Q(Graph)) (C : ConnectedComponentsData) : Q(ConnectedComponentsCertificate $G) :=
  ...
  have componentMap : Q(Batteries.RBMap (Graph.vertex $G) (Fin $val) Fin.instLinearOrder.compare) :=
    build_RBMap (C.component.map convertToPairFinVal) q(Fin.instLinearOrder)
  have component : Q(Graph.vertex $G → Fin $val) := q(fun v => Batteries.RBMap.findD $componentMap v $zeroVal)
  have componentEdge : Q(Batteries.RBSet.all (Graph.edgeSet $G) (fun x => $component (Graph.fst x) = $component (Graph.snd x)) = true) := (q(Eq.refl true) : Lean.Expr)
  have convertToPairValFin : Nat × Nat → Q(Fin $val × Fin $n) := fun (i,j) => q(($(finOfData val i), $(finOfData n j)))
  have rootMap : Q(Batteries.RBMap (Fin $val) (Graph.vertex $G) Fin.instLinearOrder.compare) :=
    build_RBMap (C.root.map convertToPairValFin) q(Fin.instLinearOrder)
  have root : Q(Fin $val → Graph.vertex $G) := q(fun v => Batteries.RBMap.findD $rootMap v $zeroVert)
  ...
  have nextMap : Q(Batteries.RBMap (Graph.vertex $G) (Graph.vertex $G) Fin.instLinearOrder.compare) :=
    build_RBMap (C.next.map convertToPairOfFins) q(Fin.instLinearOrder)
  have next : Q(Graph.vertex $G → Graph.vertex $G) := q(fun v => Batteries.RBMap.findD $nextMap v v)
  have convertToFinNat : Nat × Nat → Q(Fin $n × Nat) := fun (i,j) => q(($(finOfData n i), $j))
  have distToRootMap : Q(Batteries.RBMap (Graph.vertex $G) Nat Fin.instLinearOrder.compare) :=
    build_RBMap (C.distToRoot.map convertToFinNat) q(Fin.instLinearOrder)
  have distToRoot : Q(Graph.vertex $G → Nat) := q(fun v => Batteries.RBMap.findD $distToRootMap v 0)
